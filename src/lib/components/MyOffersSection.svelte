...
<div class="w-full flex flex-col justify-center mt-8 items-center pb-20 " >
    ...
    <h3 class="text-center text-xl lg:text-3xl font-black pb-6">ich mache ... </h3>
    <h2 class="text-center text-5xl lg:text-8xl font-medium mt-4 text-primary-200">
        {offers[activeOffer].title}
    </h2>
    <div class="text-center md:text-3xl text-xl font-semibold text-gray-300 mt-3">
        {offers[activeOffer].text}
    </div>
    <div class="flex flex-wrap justify-center mt-12 gap-4">
        {#each offers as {title},index}

<OfferingsButton {index} {title} {changeActiveOffer} {activeOffer}></OfferingsButton>
        {/each}
    </div></div>
</div>
